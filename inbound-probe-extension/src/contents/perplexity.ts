...
async function submitQuestion(question: string, adapter: PlatformAdapterConfig): Promise<void> {
  ...
  const inputSelector = selectors.input ?? "textarea"
...
function installCaptureHooks(
  ...
  const chatApi = adapter.apiPatterns?.chatApi ?? "/api/chat"
  let settled = false
  ...
  return new Promise((resolve, reject) => {
    ...
    const patchedFetch: typeof window.fetch = async (...args) => {
      ...
      try {
        ...
        if (urlMatchesChatApi(url, chatApi)) {
          const clone = response.clone()
          void clone
            .json()
            .then(onPayload)
            .catch(() => clone.text().then((t) => {
              try {
                onPayload(JSON.parse(t))
              } catch {
                /* ignore non-json */
              }
            }))
    ...
    class PatchedEventSource extends OriginalEventSource {
      constructor(url: string | URL, init?: EventSourceInit) {
        ...
        this.addEventListener("message", (ev) => {
          try {
            onPayload(JSON.parse(String(ev.data)))
          } catch {
            /* ignore */
          }
        })
